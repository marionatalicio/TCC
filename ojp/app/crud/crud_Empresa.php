...
class crud_empresa  {
    ...
    public function get($id){
		$consulta = "SELECT * FROM empregador where CNPJ = '$id'";
		$consulta = $this->conexao->query($consulta);
		return $consulta;
    }
    public function deletar($id){
		$consulta = "DELETE FROM empregador where CNPJ = '$id'";
		$this->conexao->exec($sql);
    }
     public function editar($empresa){
		$sql = "UPDATE empregador set nome = '$empresa->nome', data = '$empresa->data', RG = '$empresa->RG', CPF = '$empresa->CPF', telefone = '$empresa->telefone', cidade = '$empresa->cidade', bairro = '$empresa->bairro', rua = '$empresa->rua', numero = '$empresa->numero', cep = '$empresa->cep', email = '$empresa->email', imagem = '$empresa->imagem', login = '$empresa->login', senha = '$empresa->senha' where id_empregador = '$empresa->id'";
		$this->conexao->exec($sql);
    }
	public function buscaArea(){
		$consulta_area = "SELECT area from empregador";
